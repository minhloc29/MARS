"""Neural layers ported from SIL/CVRP/Train/VRPModel.py.

Luo et al., ICLR 2025, Boosting Neural Combinatorial Optimization for
Large-Scale Vehicle Routing Problems. See README.md for provenance.
Original layer names, detached customer embeddings and probability smoothing
are retained. The policy supplies a scoped torch.device context for factories.
"""
import torch
    ...
    def forward(self, data, problems,selected_node_list, current_step,capacity, remaining_capacity):
        ...
        if new_data_len > knearest_num:
            ...
            index_1_ = torch.arange(batch_size_V, dtype=torch.long)[:, None].expand(batch_size_V,
                                                                                    sort_index.shape[
                                                                                        1] * 2)
            index_2_ = torch.cat(((sort_index).type(torch.long), (left_node.shape[1]) + (sort_index).type(torch.long)),
                                 dim=-1)
