...
class POMOSlot(POMO):
    ...
    def __init__(
        ...
        **pomo_kwargs,
        ...
        log.info(
            ...
            f"problem={problem}, has_depot={has_depot}, disable_slots={disable_slots}"
        )
    ...
    def shared_step(
        ...
        # Node indexing follows the SlotInjectingEncoder:
        #   CVRP: A_ik covers hidden[:, 1:, :] (N-1 customers); d_ins covers the same nodes.
        #   TSP:  A_ik covers hidden (all N nodes); d_ins covers all N nodes; there is no
        #         depot, so d_ins uses a synthetic center pseudo-depot (depot_loc=None).
        if (
